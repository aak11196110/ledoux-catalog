// ═══════════════════════════════════════════
//  LEDOUX 諾科照明 報價系統 — Part 1
//  常數 / CSS / Icon / Helpers / PDF 產生器
// ═══════════════════════════════════════════
...
// ── 差旅條款門檻 ──
const TRAVEL_STAY = {
  kmThreshold:   150,   // 單程超過 → 需住宿
  lampThreshold: 80,    // 單日盞數超過 → 需住宿
  stayPerNight:  2000,  // 每人每晚 NT$
  mealPerDay:    500,   // 每人每日誤餐費 NT$
};
...
// ── SVG Icon 元件（細線幾何，取代卡通 emoji）──
const BagIcon = ({size=18}) => (
  <svg width={size} height={size} viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round">
    <rect x="2.5" y="6" width="13" height="10" rx="0.5"/>
    <path d="M6 6V5a3 3 0 0 1 6 0v1"/>
  </svg>
);
const FlaskIcon = ({size=18}) => (
  <svg width={size} height={size} viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round">
    <path d="M7 2h4M6.5 8.5 3.5 15.5h11l-3-7V2h-4v6.5z"/>
    <circle cx="8" cy="12" r="0.7" fill="currentColor"/>
    <circle cx="10.5" cy="13.5" r="0.5" fill="currentColor"/>
  </svg>
);
const ToolIcon = ({size=18}) => (
  <svg width={size} height={size} viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round">
    <path d="M15 3c.3 1.5-.7 3-2.2 3.5L6 13.5A1.4 1.4 0 0 1 4 11.5l7-7.5C12.5 3.5 13.8 2.7 15 3Z"/>
    <circle cx="5" cy="13" r="1.2"/>
  </svg>
);
const SearchIcon = ({size=14}) => (
  <svg width={size} height={size} viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round">
    <circle cx="6" cy="6" r="4.5"/>
    <line x1="9.5" y1="9.5" x2="13" y2="13"/>
  </svg>
);
const CloseIcon = ({size=12}) => (
  <svg width={size} height={size} viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round">
    <line x1="1.5" y1="1.5" x2="10.5" y2="10.5"/>
    <line x1="10.5" y1="1.5" x2="1.5" y2="10.5"/>
  </svg>
);
const PlaceholderIcon = () => (
  <svg width="44" height="44" viewBox="0 0 44 44" fill="none" stroke="var(--bdr)" strokeWidth="0.5">
    <circle cx="22" cy="22" r="18"/>
    <circle cx="22" cy="22" r="7"/>
    <line x1="22" y1="4" x2="22" y2="15"/>
    <line x1="22" y1="29" x2="22" y2="40"/>
    <line x1="4" y1="22" x2="15" y2="22"/>
    <line x1="29" y1="22" x2="40" y2="22"/>
  </svg>
);
...
function Carousel({ images }) {
  ...
  if (!imgs.length) return (
    <div className="car"><PlaceholderIcon/></div>
  );
  return (
    <div className="car">
...
function generatePDF({ cart, projectName, customer, isVip, installCalc }) {
  ...
  const subtotal   = cart.reduce((s,it)=>s+(isVip?it.product.projPrice:it.product.stdPrice)*it.qty, 0);
  const needShip   = subtotal < 3000;
  const shipFee    = needShip ? cart.reduce((s,it)=>s+(it.product.shipping||90)*it.qty, 0) : 0;
  const instTotal  = installCalc&&!installCalc.hasVHigh&&installCalc.travelFee!==null ? installCalc.laborTotal+(installCalc.travelFee||0) : 0;
  const tax        = Math.round(subtotal*0.05);
  const total      = subtotal + tax + shipFee + instTotal;
  const rows       = cart.map((item,i)=>{
    const p = isVip?item.product.projPrice:item.product.stdPrice;
    return `<tr><td>${i+1}</td><td><b>${item.product.model}</b><br><span style="font-size:10px;color:#666">${item.product.series}</span></td><td>${item.product.watt||""}</td><td>${item.product.beam||""}</td><td>${item.product.cct||""}</td><td>${item.product.voltage||""}</td><td>${item.product.cri||""}</td><td style="text-align:center">${item.qty}</td><td>組</td><td style="text-align:right">NT$ ${p.toLocaleString()}</td><td style="text-align:right">NT$ ${(p*item.qty).toLocaleString()}</td></tr>`;
  }).join("");
  const html = `<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><title>報價單 ${qn}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:"Noto Sans TC","Microsoft JhengHei",sans-serif;font-size:12px;color:#111;padding:28px 36px}.hd{display:flex;justify-content:space-between;margin-bottom:18px;border-bottom:2px solid #111;padding-bottom:12px}.co-name{font-size:18px;font-weight:700;letter-spacing:2px}.co-sub{font-size:10px;color:#555;margin-top:2px}.doc-title{text-align:center;font-size:16px;font-weight:700;letter-spacing:4px;margin-bottom:14px}.meta{display:grid;grid-template-columns:1fr 1fr;border:1px solid #ccc;margin-bottom:14px}.mc{padding:7px 10px;border-bottom:1px solid #ccc;font-size:11px}.mc:nth-child(odd){border-right:1px solid #ccc}.ml{font-size:9px;color:#666;letter-spacing:1px}.mv{font-weight:500;margin-top:1px}table{width:100%;border-collapse:collapse;margin-bottom:12px;font-size:11px}th{background:#111;color:#fff;padding:6px 7px;text-align:left;font-size:9px}td{padding:6px 7px;border-bottom:1px solid #e0e0e0;vertical-align:top}tr:nth-child(even) td{background:#fafafa}.tot{display:flex;justify-content:flex-end;margin-bottom:14px}.tt{border:1px solid #ccc;min-width:240px}.tr{display:flex;justify-content:space-between;padding:7px 12px;border-bottom:1px solid #eee}.tr.bold{font-weight:700;font-size:13px;background:#f5f5f5}.notes{border:1px solid #ccc;padding:12px;margin-bottom:14px;font-size:10px;line-height:2;color:#444}.sign{display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-top:24px}.sb{border-top:1px solid #ccc;padding-top:6px;font-size:10px;color:#666}.footer{margin-top:18px;border-top:1px solid #ccc;padding-top:8px;font-size:9px;color:#999;text-align:center}</style></head><body>
...
<table><thead><tr><th>#</th><th>型號／系列</th><th>瓦數</th><th>角度</th><th>色溫</th><th>電壓</th><th>演色性</th><th>數量</th><th>單位</th><th>單價</th><th>金額</th></tr></thead><tbody>${rows}</tbody></table>
<div class="tot"><div class="tt">${needShip?`<div class="tr"><span>運費</span><span>NT$ ${shipFee.toLocaleString()}</span></div>`:""}<div class="tr"><span>稅金（5%）</span><span>NT$ ${tax.toLocaleString()}</span></div>${instTotal>0?`<div class="tr"><span>安裝服務費</span><span>NT$ ${instTotal.toLocaleString()}</span></div>`:""}<div class="tr bold"><span>總計</span><span>NT$ ${total.toLocaleString()}</span></div></div></div>
...
export default function App() {
  ...
  const doSearch = q => {
    setSearchQ(q);
    if (q.trim()&&!searchHist.includes(q)) setSearchHist(h=>[q,...h].slice(0,8));
  ...
  const doApprove = () => {
    ...
    setMembers(x=>[...x,{...approveT,...approveF,status:"approved",approvedAt:new Date().toISOString().split("T")[0]}]);
    setPending(x=>x.filter(p=>p.id!==approveT.id));
    setApproveT(null); toast$("帳號已開通");
  };
  ...
  const handleGenPDF = () => {
    ...
    generatePDF({cart,projectName:projName,customer:{company:user.company,name:user.name,position:user.position},isVip,installCalc:instRegion?instCalc:null});
    toast$("報價單已下載");
  };
  ...
  if (autoAdmin) return (
    ...
    <div className="info-page"><div className="info-card">
      ...
      <div style={{borderTop:"0.5px solid var(--bdr2)"}}>
        {[["姓名",autoAdmin.name],["公司",autoAdmin.company],["帳號",autoAdmin.username],["身份","管理者"]].map(([k,v])=>(
          <div key={k} className="info-row"><span>{k}</span><span style={k==="身份"?{color:"var(--gold)"}:{}}>{v}</span></div>
        ))}
      </div>
  ...
  if (waitInfo) return (
    ...
    <div className="info-page"><div className="info-card">
      ...
      <div className="info-desc">感謝您的申請，審核後將開通帳號。</div>
      <div style={{borderTop:"0.5px solid var(--bdr2)"}}>
        {[["姓名",waitInfo.name],["公司",waitInfo.company],["帳號",waitInfo.username],["申請日",waitInfo.appliedAt]].map(([k,v])=>(
    ...
    <div className="auth-page">
      <div className="auth-visual">
        ...
        <div style={{marginTop:"auto",position:"relative",zIndex:1}}>
          ...
          <div className="av-sub">Authorized Partners Only</div>
        </div>
      </div>
        ...
        <div className="auth-inner">
          ...
          {authTab==="login"?(
            <>
              ...
              <button className="btn-primary" onClick={doLogin}>進入系統</button>
              {loginErr&&<div className="auth-err">{loginErr}</div>}
              <div className="auth-hint">本系統為 Ledoux Taiwan 授權合作夥伴專屬平台<br/>帳號需由專員審核後開通</div>
            </>
          ):(
            <>
              {isFirst&&<div className="first-notice">本系統尚無帳號，您將自動成為第一位管理者</div>}
              <div className="r2">
              ...
              <div className="r2">
                <div className="lf"><label>統一編號</label><input value={regF.taxId} onChange={e=>{setRegF(p=>({...p,taxId:e.target.value}));setFe(p=>({...p,taxId:""}));}} placeholder="選填 8 位數字" maxLength={8}/>{fe.taxId&&<div className="ferr">{fe.taxId}</div>}</div>
    ...
    <div className="app">
      ...
      <div className={`sidemenu ${menuOpen?"open":""}`}>
        <div className="sm-head">
          ...
          <button className="sm-close-btn" onClick={()=>setMenuOpen(false)}><CloseIcon/></button>
        </div>
        ...
        <div className="sm-nav">
          ...
          {[{id:"catalog",label:"產品目錄"},{id:"inquiry",label:"詢價單",badge:cartCount},{id:"sample",label:"借樣品",badge:sampCart.length},{id:"install",label:"安裝服務"}].map(n=>(
          ...
          {user.role==="admin"&&<>
            ...
            {[{id:"pending",label:"待審核",badge:pending.length},{id:"members",label:"帳號管理"},{id:"products",label:"產品管理"},{id:"sample_admin",label:"樣品申請",badge:sampleReqs.filter(r=>r.status==="pending").length},{id:"install_admin",label:"安裝申請",badge:installOrd.filter(o=>o.status==="pending").length}].map(n=>(
              <div key={n.id} className={`sm-item ${page===n.id?"on":""}`} onClick={()=>{setPage(n.id);setMenuOpen(false);}}>
                <span>{n.label}</span>{n.badge>0&&<span className="sm-badge">{n.badge}</span>}
              </div>
            ))}
      ...
      <nav className="topnav">
        ...
        <div className="tn-right">
          ...
          <div className="tn-user-info">
            ...
            <div className="tn-ucomp">{user.company}</div>
          </div>
      ...
      <div className="content">
        ...
        {page==="catalog"&&<>
          <div className="phead">
            <div>
              <div className="ptitle">{seriesF||searchQ?"":""}{seriesF?seriesF:searchQ?"搜索結果":"產品目錄"}</div>
              ...
              <div key={p.id} className="pcard" onClick={()=>setSelProd(p)}>
                ...
                <div className="pcard-body">
                  ...
                  <div className="pcard-tags">
                    ...
                    {p.video&&<span className="ptag vtag">影片</span>}
                  </div>
                  <div className="pcard-price">
                    {isVip?<div className="price-val gold">NT$ {p.projPrice?.toLocaleString()}</div>
                      :(p.stdPrice>0?<div className="price-val">NT$ {p.stdPrice?.toLocaleString()}</div>
          ...
          {cart.length===0?<div className="empty">請至產品目錄加入品項</div>:<>
            <div className="tbl-wrap"><table>
              ...
              <tbody>{cart.map(item=>{
                ...
                return <tr key={item.product.id}>
                  <td style={{fontWeight:400}}>{item.product.model}</td><td>{item.product.series}</td><td>{item.product.watt}</td>
                  <td><div style={{display:"flex",alignItems:"center",gap:6}}><button className="qty-btn" onClick={()=>updateQty(item.product.id,-1)}>−</button><span>{item.qty}</span><button className="qty-btn" onClick={()=>updateQty(item.product.id,1)}>+</button></div></td>
                  <td style={{color:isVip?"var(--gold)":"inherit"}}>NT$ {price?.toLocaleString()}</td>
                  <td>NT$ {(price*item.qty).toLocaleString()}</td>
                  <td><button className="btn-del2" onClick={()=>removeItem(item.product.id)}><CloseIcon/></button></td>
                </tr>;
              })}</tbody>
            </table></div>
            <div style={{maxWidth:460}}>
              ...
              <div className="checklist">
                ...
                {[{k:"c1",t:"單筆未滿 NT$3,000 運費由買方自付"},{k:"c2",t:"庫存不足時生產交期約 4 週起"},{k:"c3",t:"保固室內 3 年、戶外 2 年"},{k:"c4",t:"報價單有效期 30 天請回簽確認"}].map(({k,t})=>(
                  <label key={k} className="cl-item"><input type="checkbox" checked={checks[k]} onChange={e=>setChecks(p=>({...p,[k]:e.target.checked}))}/>{t}</label>
                ))}
              </div>
          ...
          <div className="pgrid">
            {products.map(p=>(
              <div key={p.id} className="pcard">
                <div className="pcard-img" onClick={()=>setSelProd(p)} style={{cursor:"pointer"}}>{p.images?.[0]?<img src={p.images[0]} alt={p.model}/>:<PlaceholderIcon/>}</div>
                <div className="pcard-body">
                  <div className="pcard-series">{p.series}</div>
                  <div className="pcard-model">{p.model}</div>
                  <div className="pcard-tags">{p.watt&&<span className="ptag">{p.watt}</span>}{p.beam&&<span className="ptag">{p.beam}</span>}</div>
                  <button className={`btn-samp ${sampCart.find(i=>i.id===p.id)?"done":""}`} onClick={()=>sampCart.find(i=>i.id===p.id)?removeSamp(p.id):addToSamp(p)}>
                    {sampCart.find(i=>i.id===p.id)?"已加入":"申請樣品"}
                  </button>
                </div>
              </div>
            ))}
        ...
        {page==="install"&&<>
          ...
          <div className="hint-box">
            <strong style={{color:"var(--blk)",display:"block",marginBottom:4}}>服務範疇</strong>
            本服務提供燈具定位與安裝，<strong>不含開孔、線路抽換、木作或補漆</strong>。出發地：桃園市八德區。停車費由客戶依收據實報實銷。
          </div>
          {/* 費率卡 */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:1,background:"var(--bdr2)",border:"0.5px solid var(--bdr2)",marginBottom:28}}>
            {[
              ["最低出勤費","NT$ 2,000","單次工資未達此標，以 2,000 計收"],
              ["標準安裝（3m 內）","NT$ 200 / 盞","含定位、安裝與功能測試"],
              ["挑高施工（3.1–4.5m）","+NT$ 100–200 / 盞","視現場難度，需 A 型梯"],
              ["4.5m 以上","安全紅線","需鷹架或高空作業車，費用另計"],
            ].map(([t,v,d])=>(
              <div key={t} style={{background:"var(--ivory)",padding:"18px 20px"}}>
                ...
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,color:t.includes("4.5")?"var(--red)":"var(--blk)",marginBottom:4}}>{v}</div>
          ...
          <div style={{marginBottom:28}}>
            ...
            <div className="tbl-wrap">
              <table className="install-tbl">
                <thead><tr><th>服務區域</th><th>代表縣市</th><th>里程參考</th><th>基礎車馬費</th><th>免收門檻</th></tr></thead>
                <tbody>{INSTALL_REGIONS.map(r=>(
                  <tr key={r.id}>
                    <td style={{fontWeight:400}}>{r.label}</td>
                    <td style={{color:"var(--muted)",fontSize:10}}>{r.areas}</td>
                    <td style={{color:"var(--muted)",fontSize:10}}>{r.km}</td>
                    <td style={{fontFamily:"'Cormorant Garamond',serif",fontSize:14}}>{r.travel===null?"專案另議":`NT$ ${r.travel.toLocaleString()}`}</td>
                    <td style={{fontSize:10,color:"var(--green)"}}>{r.freeAt?`${r.freeAt} 盞免收`:r.travel===null?"不設門檻":"—"}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
          ...
          <div className="hint-box" style={{marginBottom:24}}>
            <strong style={{color:"var(--blk)",display:"block",marginBottom:4}}>遠程差旅與過夜施工條款</strong>
            為守護每一盞燈的安裝精度與技師體力，長途專案建議採跨日施工。<br/>
            符合下列任一條件者，需另計差旅費：<br/>
            · 單程超過 <strong>150 公里</strong>（雲林以南、宜蘭南端、花東）<br/>
            · 單日安裝超過 <strong>80 盞</strong> 或預估總工時超過 <strong>10 小時</strong><br/><br/>
            住宿費由客戶負擔（實報實銷或定額 NT$2,000/人/晚）<br/>
            跨日施工酌收誤餐費 NT$500/人/日
          </div>
          <div style={{textAlign:"center",paddingTop:14,borderTop:"0.5px solid var(--bdr2)"}}>
            <button className="btn-primary" style={{maxWidth:260,margin:"0 auto",display:"block"}} onClick={()=>setInstOpen(true)}>立即估算安裝費用</button>
          </div>
        </>}
        ...
        {page==="pending"&&user.role==="admin"&&<>
          ...
          {approveT&&<div className="modal-wrap" onClick={()=>setApproveT(null)}><div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-head"><div className="modal-title">核准申請</div><button className="close-btn" onClick={()=>setApproveT(null)}><CloseIcon/></button></div>
            <div className="modal-body">
              <div className="ac-card"><div className="ac-name">{approveT.name} · {approveT.position}</div><div className="ac-detail">{approveT.company}<br/>{approveT.email} · {approveT.phone}</div></div>
              <div className="fgrid">
                <div className="ff"><label>登入帳號</label><input value={approveF.username} onChange={e=>setApproveF(p=>({...p,username:e.target.value}))} placeholder={approveT.username}/></div>
              ...
              <div className="form-actions"><button className="btn-confirm" onClick={doApprove}>確認核准</button><button className="btn-cancel-sm" onClick={()=>setApproveT(null)}>取消</button></div>
            </div>
          </div></div>}
          {pending.length===0?<div className="empty">目前沒有待審核的申請</div>:
            <div className="tbl-wrap"><table><thead><tr><th>姓名</th><th>職稱</th><th>公司</th><th>電話</th><th>申請日</th><th>操作</th></tr></thead>
              <tbody>{pending.map(p=><tr key={p.id}><td style={{fontWeight:400}}>{p.name}</td><td style={{color:"var(--muted)"}}>{p.position}</td><td>{p.company}</td><td style={{color:"var(--muted)"}}>{p.phone}</td><td style={{color:"var(--muted)"}}>{p.appliedAt}</td><td style={{display:"flex",gap:6}}><button className="btn-ok" onClick={()=>{setApproveT(p);setApproveF({username:p.username,password:p.password,role:"standard"});}}>核准</button><button className="btn-ng" onClick={()=>{setPending(x=>x.filter(x=>x.id!==p.id));toast$("已拒絕");}}>拒絕</button></td></tr>)}</tbody>
            </table></div>}
        </>}
        ...
        {page==="members"&&user.role==="admin"&&<>
          ...
          <div className="stats-row">
            {[["總帳號",members.length,""],["管理者",members.filter(m=>m.role==="admin").length,""],["VIP",members.filter(m=>m.role==="vip").length,""],["待審核",pending.length,"var(--red)"]].map(([l,n,c])=>(
              <div key={l} className="stat-box"><div className="stat-num" style={c?{color:c}:{}}>{n}</div><div className="stat-lbl">{l}</div></div>
            ))}
          </div>
          <div className="tbl-wrap"><table>
            <thead><tr><th>姓名</th><th>公司</th><th>帳號</th><th>密碼</th><th>身份</th><th>調整</th><th>開通日</th><th></th></tr></thead>
            <tbody>{members.map(m=>(
              <tr key={m.id}>
                <td style={{fontWeight:400}}>{m.name}</td><td>{m.company}</td>
                <td style={{fontFamily:"monospace"}}>{m.username}</td>
                <td style={{fontFamily:"monospace",color:"var(--muted)"}}>{m.password}</td>
                <td><span className={`rb r-${m.role==="admin"?"admin":m.role==="vip"?"vip":"std"}`}>{roleLabel(m.role)}</span></td>
                <td><select className="role-sel" value={m.role} onChange={e=>{setMembers(x=>x.map(i=>i.id===m.id?{...i,role:e.target.value}:i));if(user.id===m.id)setUser(u=>({...u,role:e.target.value}));toast$("權限已更新");}}><option value="standard">一般</option><option value="vip">VIP</option><option value="admin">管理</option></select></td>
                <td style={{color:"var(--muted)"}}>{m.approvedAt}</td>
                <td>{m.id!==user.id&&<button className="btn-del2" onClick={()=>{setMembers(x=>x.filter(x=>x.id!==m.id));toast$("帳號已刪除");}}><CloseIcon/></button>}</td>
              </tr>
            ))}</tbody>
          </table></div>
        </>}
        ...
        {page==="products"&&user.role==="admin"&&<>
          ...
          <div className="hint-box">圖片請貼入圖片網址（每行一個）。教學影片請貼入 YouTube Embed URL：https://www.youtube.com/embed/影片ID</div>
          {showAdd&&<div className="form-panel"><div className="fp-title">新增產品</div>
            <div className="fgrid">
              {[["型號","model"],["系列","series"],["瓦數","watt"],["色溫","cct"],["光束角","beam"],["電壓","voltage"],["演色性","cri"],["顏色","color"],["開孔尺寸","cutout"],["產品尺寸","size"],["認證","cert"],["標準價","stdPrice"],["專案價","projPrice"],["運費","shipping"]].map(([l,k])=>(
                <div key={k} className="ff"><label>{l}</label><input type={["stdPrice","projPrice","shipping"].includes(k)?"number":"text"} value={newProd[k]} onChange={e=>setNewProd(p=>({...p,[k]:e.target.value}))}/></div>
              ))}
          ...
          {editProd&&<div className="form-panel" style={{background:"#f0ebe2"}}><div className="fp-title">編輯：{editProd.model}</div>
            <div className="fgrid">
              {[["型號","model"],["系列","series"],["瓦數","watt"],["色溫","cct"],["光束角","beam"],["電壓","voltage"],["演色性","cri"],["顏色","color"],["開孔尺寸","cutout"],["產品尺寸","size"],["認證","cert"],["標準價","stdPrice"],["專案價","projPrice"],["運費","shipping"]].map(([l,k])=>(
                <div key={k} className="ff"><label>{l}</label><input type={["stdPrice","projPrice","shipping"].includes(k)?"number":"text"} value={editProd[k]||""} onChange={e=>setEditProd(p=>({...p,[k]:e.target.value}))}/></div>
              ))}
          ...
          <div className="tbl-wrap"><table>
            <thead><tr><th>型號</th><th>系列</th><th>分類</th><th>瓦數</th><th>標準價</th><th>專案價</th><th>影片</th><th>操作</th></tr></thead>
            <tbody>{products.map(p=>(
              <tr key={p.id}>
                <td style={{fontWeight:400}}>{p.model}</td><td>{p.series}</td><td>{p.category}</td><td>{p.watt}</td>
                <td>NT$ {p.stdPrice?.toLocaleString()}</td>
                <td style={{color:"var(--gold)"}}>NT$ {p.projPrice?.toLocaleString()}</td>
                <td>{p.video?<span style={{fontSize:10,color:"var(--green)"}}>✓</span>:<span style={{color:"var(--muted)"}}>—</span>}</td>
                <td style={{display:"flex",gap:6}}><button className="btn-edit2" onClick={()=>startEdit(p)}>編輯</button><button className="btn-del2" onClick={()=>{setProducts(x=>x.filter(x=>x.id!==p.id));toast$("已刪除");}}><CloseIcon/></button></td>
              </tr>
            ))}</tbody>
          </table></div>
        </>}
        ...
        {page==="sample_admin"&&user.role==="admin"&&<>
          ...
          {sampleReqs.length===0?<div className="empty">目前沒有樣品申請</div>:
            <div className="tbl-wrap"><table>
              <thead><tr><th>日期</th><th>聯絡人</th><th>公司</th><th>電話</th><th>品項</th><th>狀態</th><th>操作</th></tr></thead>
              <tbody>{sampleReqs.map(r=>(
                <tr key={r.id}>
                  <td style={{color:"var(--muted)"}}>{r.date}</td>
                  <td style={{fontWeight:400}}>{r.form.name}</td><td>{r.form.company}</td>
                  <td style={{color:"var(--muted)"}}>{r.form.phone}</td>
                  <td style={{fontSize:10}}>{r.products.join("、")}</td>
                  <td><span className={`rb ${r.status==="pending"?"r-std":"r-vip"}`}>{r.status==="pending"?"待處理":"已處理"}</span></td>
                  <td><button className="btn-ok" onClick={()=>setSampleReqs(x=>x.map(i=>i.id===r.id?{...i,status:"done"}:i))}>完成</button></td>
                </tr>
              ))}</tbody>
            </table></div>}
        </>}
        ...
        {page==="install_admin"&&user.role==="admin"&&<>
          ...
          {installOrd.length===0?<div className="empty">目前沒有安裝申請</div>:
            <div className="tbl-wrap"><table>
              <thead><tr><th>日期</th><th>客戶</th><th>區域</th><th>盞數</th><th>預估費用</th><th>差旅</th><th>狀態</th><th>操作</th></tr></thead>
              <tbody>{installOrd.map(o=>{
                const qty=o.groups.reduce((s,g)=>s+Number(g.qty||0),0);
                ...
                return <tr key={o.id}>
                  ...
                  <td style={{fontWeight:400}}>{o.customer.name}<br/><span style={{fontSize:9,color:"var(--muted)"}}>{o.customer.company}</span></td>
                  <td>{INSTALL_REGIONS.find(r=>r.id===o.region)?.label}</td>
                  <td style={{textAlign:"center"}}>{qty}</td>
                  <td style={{fontFamily:"'Cormorant Garamond',serif",fontSize:14,color:"var(--gold)"}}>{est?`NT$ ${est.toLocaleString()}`:"另議"}</td>
                  <td>{o.calc?.needStay?<span style={{fontSize:9,color:"var(--red)"}}>需住宿</span>:"—"}</td>
                  <td><span className={`rb ${o.status==="pending"?"r-std":"r-vip"}`}>{o.status==="pending"?"待確認":"已處理"}</span></td>
                  <td><button className="btn-ok" onClick={()=>setInstallOrd(x=>x.map(i=>i.id===o.id?{...i,status:"done"}:i))}>完成</button></td>
                </tr>;
              })}</tbody>
            </table></div>}
        </>}
        ...
        <div className="drawer" onClick={e=>e.stopPropagation()}>
          ...
          <div className="drawer-body">
            ...
            <div className="spec-grid">
              {[["瓦數",selProd.watt],["流明",selProd.lumen],["色溫",selProd.cct],["光束角",selProd.beam],["電壓",selProd.voltage],["演色性",selProd.cri],["顏色",selProd.color],["開孔尺寸",selProd.cutout],["產品尺寸",selProd.size],["安裝方式",selProd.install],["認證",selProd.cert]].filter(([,v])=>v&&v!=="—").map(([l,v])=>(
                <div key={l} className="spec-item"><div className="spec-label">{l}</div><div className="spec-val">{v}</div></div>
              ))}
            </div>
            {/* 教學影片 */}
            {selProd.video&&(
              <div style={{marginBottom:16}}>
                <div style={{fontSize:"7px",letterSpacing:"4px",textTransform:"uppercase",color:"var(--muted)",marginBottom:8}}>教學影片</div>
                <div className="drawer-video"><iframe src={selProd.video} title="教學影片" allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture" allowFullScreen/></div>
              </div>
            )}
            {/* 燈帶安裝提醒 */}
            {(selProd.category==="鋁條燈"||selProd.category==="矽膠燈帶")&&(
              <div style={{background:"#fff8ee",border:"0.5px solid #e8c97a",borderLeft:"1px solid var(--gold)",padding:"10px 14px",marginBottom:14,fontSize:11,lineHeight:1.8,color:"#6a5a3a"}}>
                <strong style={{color:"var(--gold)",display:"block",marginBottom:3}}>安裝注意事項</strong>
                {selProd.category==="鋁條燈"?<>
                  單條建議最長 <strong>2m</strong> · 串聯最長 <strong>5.5m</strong><br/>需搭配 <strong>DC 24V 恒壓電源</strong>
                </>:<>
                  單條最長 <strong>5m</strong> · 超過請分段供電<br/>支援 <strong>0-10V / DMX / DALI</strong> 調光
                </>}
              </div>
            )}
            ...
            <div className="price-block">
              ...
              {isVip?<div className="pb-val gold">NT$ {selProd.projPrice?.toLocaleString()}</div>
                :(selProd.stdPrice>0?<div className="pb-val">NT$ {selProd.stdPrice?.toLocaleString()}</div>
            ...
            <div className="drawer-actions">
              ...
              <button className={`btn-samp ${sampCart.find(i=>i.id===selProd.id)?"done":""}`} onClick={()=>sampCart.find(i=>i.id===selProd.id)?removeSamp(selProd.id):addToSamp(selProd)}>
                {sampCart.find(i=>i.id===selProd.id)?"已申請樣品":"申請樣品"}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className={`side-panel ${cartOpen?"open":""}`}>
        ...
        <div className="sp-body">
          {cart.length===0?<div className="empty" style={{paddingTop:48}}>尚未加入任何產品</div>:
            cart.map(item=>{
              const price=isVip?item.product.projPrice:item.product.stdPrice;
              return <div key={item.product.id} className="ci-row">
                <div className="ci-img">{item.product.images?.[0]?<img src={item.product.images[0]} alt=""/>:<PlaceholderIcon/>}</div>
                <div className="ci-info">
                  ...
                  <div className="ci-qty">
                    ...
                    <button className="qty-btn" onClick={()=>updateQty(item.product.id,1)}>+</button>
                    <span className="ci-price" style={{marginLeft:7}}>NT$ {(price*item.qty).toLocaleString()}</span>
                  </div>
                </div>
                <button className="ci-del" onClick={()=>removeItem(item.product.id)}><CloseIcon/></button>
              </div>;
            })
          }
        </div>
        {cart.length>0&&<div className="sp-foot">
          ...
          {cart.some(i=>i.product.category==="崁燈")&&(
            <div style={{border:"0.5px solid var(--gold)",padding:"10px 12px",marginBottom:10}}>
              <div style={{fontSize:"7px",letterSpacing:"3px",textTransform:"uppercase",color:"var(--gold)",marginBottom:4}}>加購安裝服務</div>
              <div style={{fontSize:10,color:"var(--muted)",marginBottom:6}}>本訂單含 {cart.filter(i=>i.product.category==="崁燈").reduce((s,i)=>s+i.qty,0)} 盞崁燈，可一併申請安裝</div>
              <button style={{fontSize:"7px",letterSpacing:"2px",padding:"5px 10px",border:"0.5px solid var(--gold)",background:"transparent",color:"var(--gold)",cursor:"pointer",textTransform:"uppercase"}} onClick={()=>{setCartOpen(false);setInstOpen(true);}}>估算費用</button>
            </div>
          )}
          <div className="cp-project">
            <label>案名 *</label>
            <input value={projName} onChange={e=>setProjName(e.target.value)} placeholder="請輸入案名"/>
          </div>
          <div className="checklist">
            <div className="cl-title">下載前請確認</div>
            {[{k:"c1",t:"單筆未滿 NT$3,000 運費由買方自付"},{k:"c2",t:"庫存不足時生產交期約 4 週起"},{k:"c3",t:"保固室內 3 年、戶外 2 年"},{k:"c4",t:"報價單有效期 30 天請回簽確認"}].map(({k,t})=>(
      ...
      <div className={`side-panel ${sampOpen?"open":""}`}>
        ...
        <div className="sp-body">
          ...
          {sampDone?<div style={{textAlign:"center",padding:"40px 0",color:"var(--green)",fontSize:14,lineHeight:2}}>申請已送出<br/><span style={{fontSize:11,color:"var(--muted)"}}>專員將於 1-2 個工作日聯繫</span></div>:<>
            <div style={{marginBottom:14}}>
              {sampCart.length===0?<div style={{textAlign:"center",padding:"20px 0",color:"var(--muted)",fontSize:10,letterSpacing:2,textTransform:"uppercase"}}>尚未選擇樣品</div>:
                sampCart.map(p=><div key={p.id} className="samp-item">
                  <div className="samp-item-img">{p.images?.[0]?<img src={p.images[0]} alt=""/>:<PlaceholderIcon/>}</div>
                  <div className="samp-item-info"><div className="samp-item-model">{p.model}</div><div className="samp-item-sub">{p.series} · {p.watt}</div></div>
                  <button className="samp-item-del" onClick={()=>removeSamp(p.id)}><CloseIcon/></button>
                </div>)
              }
            </div>
        ...
        {!sampDone&&<div className="sp-foot">
          <button className="btn-gold" onClick={submitSamp} disabled={sampCart.length===0}>送出樣品申請</button>
          <button className="btn-ghost" onClick={()=>setSampOpen(false)}>稍後再說</button>
        </div>}
      </div>
      ...
      <div className={`side-panel ${instOpen?"open":""}`}>
        <div className="sp-head">
          <div className="sp-title">安裝費用試算</div>
          <button className="close-btn" onClick={()=>setInstOpen(false)}><CloseIcon/></button>
        </div>
        <div className="sp-body">
          {instDone?(
            ...
            <>
              ...
              <div style={{marginBottom:18}}>
                ...
                <div className="region-grid">
                  {INSTALL_REGIONS.map(r=>(
                    <div key={r.id} className={`region-card ${instRegion===r.id?"on":""}`} onClick={()=>setInstRegion(r.id)}>
                      <div className="rc-label">{r.label}</div>
                      <div className="rc-km">{r.km}</div>
                      <div className="rc-fee">{r.travel===null?"另議":`NT$ ${r.travel.toLocaleString()}`}</div>
                      {r.freeAt&&<div className="rc-free">{r.freeAt} 盞以上免收</div>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              {instCalc&&<div className="calc-box">
                ...
                {instCalc.needStay&&<div className="stay-notice">本案場符合「遠程差旅條款」，需另計住宿費（NT$2,000/人/晚）及誤餐費（NT$500/人/日）。專員確認時間時將一併說明。</div>}
              ...
              <div style={{marginTop:14}}>
                ...
                <textarea style={{width:"100%",padding:9,border:"0.5px solid var(--bdr)",background:"transparent",fontFamily:"'Noto Sans TC',sans-serif",fontSize:12,outline:"none",resize:"vertical",minHeight:56}} value={instNote} onChange={e=>setInstNote(e.target.value)} placeholder="安裝地址、偏好時段、特殊現場說明等"/>
              </div>
            </>
          )}
        </div>
        {!instDone&&<div className="sp-foot">
          <button className="btn-gold" onClick={submitInst} disabled={!instRegion}>送出安裝申請</button>
