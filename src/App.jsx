...
export default function App() {
  ...
  const saveCredential=async(u,p)=>{ if(!window.PasswordCredential)return; try{await navigator.credentials.store(new window.PasswordCredential({id:u,password:p}));}catch(_){} };
  const tryAutoLogin=async()=>{ if(!navigator.credentials){toast$("此瀏覽器不支援自動登入");return;} try{const cred=await navigator.credentials.get({password:true,mediation:"optional"});if(cred?.id){const m=members.find(m=>m.username===cred.id&&m.password===cred.password);if(m){setUser(m);toast$("✓ 自動登入成功");return;}setLoginErr("找不到對應帳號");}else toast$("未找到已儲存帳號");}catch(_){toast$("請手動登入");} };

  const doLogin=async()=>{ const m=members.find(m=>m.username===loginF.username&&m.password===loginF.password); if(m){await saveCredential(loginF.username,loginF.password);setUser(m);return;} setLoginErr("帳號或密碼錯誤"); };
